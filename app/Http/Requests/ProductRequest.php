...
class ProductRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->get('discount') && $this->get('discount_type') == 'amount') {
            $rules['discount'] = 'nullable|numeric|lt:unit_price';
        } elseif ($this->get('discount')) {
            $rules['discount'] = 'nullable|numeric|lt:100';
        } else {
            $rules['discount'] = 'nullable|numeric';
        }
        // Make current_stock not required for variant products
        if (request()->has('colors_active') && request()->has('colors') && count(request()->colors) > 0) {
            $rules['current_stock'] = 'sometimes|numeric';
        } else {
