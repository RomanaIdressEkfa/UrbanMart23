...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // Load flash deal data for the home page
        ...
        return view('frontend.' . get_setting('homepage_select') . '.index', compact('featured_categories', 'lang', 'flash_deal'));
    }
    ...
    public function product(Request $request, $slug)
    {
        ...
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks.wholesalePrices', 'user', 'user.shop')->where('auction_product', 0)->where('slug', $slug)->where('approved', 1)->first();
    ...
    public function variant_price(Request $request)
    {
        ...
        // Default requested quantity to 1 if missing or invalid
        $reqQuantity = is_numeric($request->quantity ?? null) ? (int) $request->quantity : 1;

        if ($product_stock) {
            $price = $product_stock->price;

            // Wholesale pricing based on selected stock, if applicable
            if ($product->wholesale_product) {
                $wholesalePrice = $product_stock->wholesalePrices
                    ->where('min_qty', '<=', $reqQuantity)
                    ->where('max_qty', '>=', $reqQuantity)
                    ->first();
                if ($wholesalePrice) {
            ...
            $in_stock = ($quantity >= $product->min_qty) ? 1 : 0;
        } else {
            // Graceful fallback when variant stock is missing
        ...
        // Mohammad Hassan
        return array(
            'price' => single_price($price * $reqQuantity),
            'quantity' => $quantity,
...
{
    try {
        $category = Category::with('childrenCategories')->findOrFail($request->category_id);
        
        $subcategories = $category->childrenCategories->map(function($subcategory) {
            return [
                'id' => $subcategory->id,
