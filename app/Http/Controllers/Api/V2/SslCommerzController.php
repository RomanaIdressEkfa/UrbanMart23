...
class SslCommerzController extends Controller
{
    ...
    public function begin(Request $request)
    {
        ...
        $post_data['cus_phone'] = $user->phone ?? '01000000000';
        ...
        // Shipment info mapping from order or combined order
        try {
            $shipping_method = 'NO';
            $ship = [
                'ship_name' => null,
            ...
            if ($paymentType == 'cart_payment' && isset($combined_order)) {
                $addr = json_decode($combined_order->shipping_address, true);
            } elseif ($paymentType == 'order_re_payment' && isset($order)) {
            ...
            if (is_array($addr)) {
                $shipping_method = 'YES';
                $ship['ship_name'] = $addr['name'] ?? 'Customer';
                $ship['ship_add1'] = $addr['address'] ?? 'N/A';
                $ship['ship_add2'] = $addr['address_line2'] ?? null;
                $ship['ship_city'] = $addr['city'] ?? 'N/A';
                $ship['ship_state'] = $addr['state'] ?? null;
                $ship['ship_postcode'] = $addr['postal_code'] ?? '0000';
                $ship['ship_country'] = $addr['country'] ?? 'Bangladesh';
            }
