...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        // Mohammad Hassan - Pre-order statistics
        $data['total_preorders'] = Preorder::count();
        $data['pending_preorders'] = Preorder::where('request_preorder_status', 1)->count();
        $data['confirmed_preorders'] = Preorder::where('request_preorder_status', 2)->count();
        $data['arrived_preorders'] = Preorder::where('shipping_status', 2)->count();
        $data['completed_preorders'] = Preorder::where('delivery_status', 2)->count();
        $data['cancelled_preorders'] = Preorder::where('request_preorder_status', 3)->count();
        
        // Pre-order financial statistics
        $data['total_preorder_value'] = Preorder::sum('grand_total');
        $data['total_advance_paid'] = Preorder::sum('prepayment');
        $data['total_remaining_balance'] = $data['total_preorder_value'] - $data['total_advance_paid'];
        
        // This month pre-order statistics
        $data['preorders_this_month'] = Preorder::whereYear('created_at', Carbon::now()->year)
                                            ->whereMonth('created_at', Carbon::now()->month)
                                            ->count();
        $data['preorder_value_this_month'] = Preorder::whereYear('created_at', Carbon::now()->year)
                                                ->whereMonth('created_at', Carbon::now()->month)
                                                ->sum('grand_total');
