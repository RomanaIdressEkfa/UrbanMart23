...
if (!function_exists('cart_product_price')) {
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        ...
        // Prefer persisted values on the cart item first
        $price = 0;
        $source = 'derived';
        if (isset($cart_product['unit_price']) && floatval($cart_product['unit_price']) > 0) {
            $price = floatval($cart_product['unit_price']);
            $source = 'cart_unit_price';
        } elseif (isset($cart_product['price']) && floatval($cart_product['price']) > 0) {
            $price = floatval($cart_product['price']);
            $source = 'cart_price';
        }
        
        if ($product->auction_product == 0 && $source === 'derived') {
            $str = '';
            ...
            if ($product_stock) {
                ...
                // Fallback to product unit_price if no stock variant found
                $price = $product->unit_price ?? 0;
            }

            // Wholesaler price tiers
            $user = auth()->user();
            ...
            // Variant-specific discount price first, else product-level discount
            if ($product_stock && isset($product_stock->discount_price) && $product_stock->discount_price > 0) {
        ...
        } elseif ($product->auction_product != 0) {
            $price = $product->bids->max('amount');
        } else {
            // If we used cart price and not unit_price, still apply product-level discounts
            if ($source === 'cart_price') {
                $discount_applicable = false;
                if ($product->discount_start_date == null) {
                    $discount_applicable = true;
                } elseif (
...
if (!function_exists('cart_product_tax')) {
    function cart_product_tax($cart_product, $product, $formatted = true)
    {
        ...
        // Base price preference: cart unit_price -> cart price -> derived
        $price = 0;
        $source = 'derived';
        if (isset($cart_product['unit_price']) && floatval($cart_product['unit_price']) > 0) {
            $price = floatval($cart_product['unit_price']);
        ...
        if ($source === 'derived') {
            ...
            // Variant-specific discount first, else product-level
            if ($product_stock && isset($product_stock->discount_price) && $product_stock->discount_price > 0) {
                $price = $product_stock->discount_price;
            } else {
                $discount_applicable = false;
        ...
        } elseif ($source === 'cart_price') {
            // Apply product-level discount only when using cart price
            $discount_applicable = false;
            if ($product->discount_start_date == null) {
                $discount_applicable = true;
            } elseif (
...
function getShippingCost($carts, $index, $shipping_info = '', $carrier = '')
{
    ...
    if ($product == null || $product->digital == 1) {
        return 0;
    }
    ...
    if ($shipping_type == 'flat_rate') {
        // Ensure there's at least one cart item to avoid division by zero
        return count($carts) > 0 ? (get_setting('flat_rate_shipping_cost') / count($carts)) : 0;
    } 
    elseif ($shipping_type == 'seller_wise_shipping') {
        if ($product->added_by == 'admin') {
            return count($admin_products) > 0 ? (get_setting('shipping_cost_admin') / count($admin_products)) : 0;
        } else {
            $shop = Shop::where('user_id', $product->user_id)->first();
            $seller_product_count = isset($seller_products[$product->user_id]) ? count($seller_products[$product->user_id]) : 0;
            return ($shop && $seller_product_count > 0) ? ($shop->shipping_cost / $seller_product_count) : 0;
        }
    } 
    elseif ($shipping_type == 'area_wise_shipping') {
        // *** START OF THE FIX ***
        // Check for area_id first, then city_id, otherwise return 0
        if (isset($shipping_info['area_id']) && $shipping_info['area_id'] != null && $shipping_info['area_id'] != 0) {
            $area = Area::find($shipping_info['area_id']);
        } elseif (isset($shipping_info['city_id']) && $shipping_info['city_id'] != null && $shipping_info['city_id'] != 0) {
            $area = City::find($shipping_info['city_id']);
        } else {
            // If neither city_id nor area_id is set, shipping cost is 0
            return 0;
        }
        // *** END OF THE FIX ***
        ...
        if ($area != null) {
            if ($product->added_by == 'admin') {
                return count($admin_products) > 0 ? ($area->cost / count($admin_products)) : 0;
            } else {
                $seller_product_count = isset($seller_products[$product->user_id]) ? count($seller_products[$product->user_id]) : 0;
                return $seller_product_count > 0 ? ($area->cost / $seller_product_count) : 0;
            }
        }
        return 0;
    } 
    elseif ($shipping_type == 'carrier_wise_shipping') {
        $user_zone = isset($shipping_info['country_id']) && $shipping_info['country_id'] != 0 ? Country::where('id', $shipping_info['country_id'])->first()->zone_id ?? 0 : 0;
        ...
        foreach ($carrier->carrier_ranges as $carrier_range) {
            if ($itemsWeightOrPrice >= $carrier_range->delimiter1 && $itemsWeightOrPrice <= $carrier_range->delimiter2) {
                ...
                if ($product->added_by == 'admin') {
                    return count($admin_products) > 0 ? ($carrier_price / count($admin_products)) : 0;
                } else {
                    $seller_product_count = isset($seller_products[$product->user_id]) ? count($seller_products[$product->user_id]) : 0;
                    return $seller_product_count > 0 ? ($carrier_price / $seller_product_count) : 0;
                }
            }
        }
        return 0;
    } 
    else { // Product Wise Shipping
...
// Get cached featured products by category (including child categories)
if (!function_exists('get_cached_featured_products_by_category')) {
    function get_cached_featured_products_by_category($category_id, $limit = 12)
    {
        return Cache::remember('featured_products_category_' . $category_id . '_' . $limit, 3600, function () use ($category_id, $limit) {
            $category_ids = \App\Utility\CategoryUtility::children_ids($category_id);
            $category_ids[] = $category_id;

            $query = Product::query()->whereIn('category_id', $category_ids)->where('featured', 1);
            return filter_products($query)->latest()->limit($limit)->get();
        });
    }
}
...
if (!function_exists('preorder_discount_price')) {
    ...
    {
        // Use preorder_price if set, otherwise use unit_price
        $base_price = $product->preorder_price ?? $product->unit_price;
        ...
        if ($product->discount_start_date != null && $product->discount_end_date != null) {
            $discount_active = ($current_timestamp >= $product->discount_start_date && $current_timestamp <= $product->discount_end_date);
        }
        
        if ($discount_active && $product->discount > 0) {
            $discount = $product->discount;
            $discounted_price = $product->discount_type == 'flat' 
                ? $base_price - $discount 
        ...
        return max(0, $discounted_price); // Ensure price is never negative
    }
}
