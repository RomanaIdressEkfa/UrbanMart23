...
public static function order_email($order, $status)
{
    ...
    if ($order->user_id) {
        ...
        // যদি অতিথি ব্যবহারকারী হন, তাহলে shipping_address থেকে তথ্য নেওয়া হচ্ছে
    ...
    if ($seller && $seller->email != $admin->email) {
        $recipients[$seller->email] = 'seller';
    }
    if ($customer_email && $customer_email != $admin->email && (!$seller || $customer_email != $seller->email)) {
        $recipients[$customer_email] = 'customer';
    }

    foreach ($recipients as $email => $user_type) {
        $emailIdentifier = 'order_' . $status . '_email_to_' . $user_type;
        $emailTemplate = EmailTemplate::whereIdentifier($emailIdentifier)->first();

        if ($emailTemplate != null && $emailTemplate->status == 1) {
            
            $shopName = $user_type == 'seller' && $seller && $seller->shop ? $seller->shop->name : null;
            ...
            $emailBody = str_replace('[[store_name]]', get_setting('site_name'), $emailBody);
            $emailBody = str_replace('[[shop_name]]', $shopName, $emailBody);
            // অতিথি এবং রেজিস্টার্ড উভয় ব্যবহারকারীর জন্য নাম ব্যবহার করা হচ্ছে
            ...
            try {
                ...
                // লগ ফাইলে এররটি লিখে রাখা যেতে পারে
                \Log::error('Email sending failed for order ' . $order->code . ' to ' . $email . ': ' . $e->getMessage());
            }
        }
    }
