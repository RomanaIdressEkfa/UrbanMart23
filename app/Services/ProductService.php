...
class ProductService
{
    public function store(array $data)
    {
        ...
        if (!empty($collection['date_range'])) {
            ...
            if (count($date_var) >= 2) {
                $discount_start_date = strtotime(trim($date_var[0]));
                $discount_end_date   = strtotime(trim($date_var[1]));
        ...
        if (count($combinations) > 0) {
            foreach ($combinations as $key => $combination) {
                ...
                unset($collection['price_' . $str]);
                unset($collection['sku_' . $str]);
                unset($collection['qty_' . $str]);
                unset($collection['img_' . $str]);
            }
        }
        ...
        $collection['is_preorder'] = isset($collection['is_preorder']) ? 1 : 0;
