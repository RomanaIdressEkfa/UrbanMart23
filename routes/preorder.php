...
use App\Http\Controllers\Preorder\seller\PreorderProductReviewController as SellerPreorderProductReviewController;
    ...
    Route::group(['prefix' => 'admin/preorder', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function () {
        ...
        Route::controller(DashboardController::class)->group(function () {
           ...
           Route::post('/preorders/{order_code}/update-status', [\App\Http\Controllers\Admin\PreorderManagementController::class, 'updateStatus'])->name('preorders.update_status');
    ...
    Route::group(['middleware' => ['user', 'verified', 'unbanned']], function () {
            Route::controller(PreorderController::class)->group(function () {
                ...
                // Payment success and cancel should be accessible for guests; moved below
            });
...
// Guest-accessible routes for preorder (session-based flow)
    Route::group(['prefix' => 'preorder'], function () {
        // Product Details Page
        Route::get('product/{slug}',[ProductController::class,'product_details'])->name('preorder-product.details');

        Route::controller(PreorderController::class)->group(function () {
            // Main flow for guest and logged-in users
            Route::post('/direct-checkout', 'direct_checkout')->name('preorder.direct_checkout');
            Route::get('/shipping-info', 'shipping_info')->name('preorder.shipping_info');
            ...
            // AJAX route moved here to be guest-accessible
            Route::post('/updateDeliveryAddress', 'updateDeliveryAddress')->name('updateDeliveryAddress');
            ...
            // This can also be guest-accessible if needed
            Route::post('/save-shipping-info', 'save_shipping_info')->name('preorder.save_shipping_info');
        });
    });

    // Authenticated user-specific routes
    Route::group(['prefix' => 'preorder', 'middleware' => ['user', 'verified', 'unbanned']], function () {
        Route::controller(PreorderController::class)->group(function () {
            Route::post('/place-order', 'place_order')->name('preorder.place_order');
            Route::get('/order-list', 'order_list')->name('preorder.order_list');
            Route::get('/order-details/{id}', 'order_details')->name('preorder.order_details');
    ...
    // Common Routes (accessible for any logged-in user)
    ...
    // General guest routes for preorder
