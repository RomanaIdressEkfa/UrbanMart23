...
// Pickup Points API for frontend
Route::get('/pickup-points', function () {
    $pickup_points = \App\Models\PickupPoint::where('pick_up_status', 1)->get();
    return response()->json([
        'pickup_points' => $pickup_points->map(function ($point) {
            return [
                'id' => $point->id,
                'name' => $point->getTranslation('name'),
                'address' => $point->getTranslation('address'),
                'phone' => $point->phone
            ];
        })
    ]);
...
Route::controller(HomeController::class)->group(function () {
    ...
    Route::post('/category/subcategories', 'get_subcategories')->name('category.subcategories');
...
Route::controller(SslcommerzController::class)->group(function () {
    // Map pay route to controller method and name it for redirects
    Route::get('/sslcommerz/pay', 'pay')->name('sslcommerz.pay');
...
Route::group(['prefix' => 'checkout'], function () {
    Route::controller(CheckoutController::class)->group(function () {
        ...
        Route::post('/payment', 'checkout')->name('checkout.payment');
    ...
    // Graceful fallback: if someone navigates to /checkout/payment via GET, redirect to checkout
    Route::get('/payment', function () {
        return redirect()->route('checkout');
    })->name('checkout.payment_get_fallback');
});
...
Route::controller(BkashController::class)->group(function () {
    // Add explicit payment init route used by PreorderController
    Route::get('/bkash/pay', 'pay')->name('bkash.payment');
