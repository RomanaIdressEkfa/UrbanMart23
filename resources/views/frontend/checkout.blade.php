...
@section('content')
    ...
    <section class="py-2 gry-bg">
        <div class="container-fluid">
            <div class="row cols-xs-space cols-sm-space cols-md-space">
                <div class="col-12">  {{-- <<< THIS LINE WAS CHANGED --}}
                    <form class="form-default" action="{{ route('checkout.payment') }}" role="form" method="POST" id="checkout-form">
                        @csrf
                            ...
                            <div class="col-lg-4 mt-4 mt-lg-0" id="cart_summary">
                                <div class="card shadow-sm border-0 rounded-0">
                                    ...
                                    <div class="pt-3 px-4 fs-14">
                                        ...
                                        <a href="{{ route('terms') }}" class="fw-600">{{ translate('terms and conditions') }}</a>,
                                        <a href="{{ route('returnpolicy') }}" class="fw-600">{{ translate('return policy') }}</a> &
                                        <a href="{{ route('privacypolicy') }}" class="fw-600">{{ translate('privacy policy') }}</a>
                                    </div>

                                    <div class="row align-items-center pt-3 px-4 pb-4">
                                        <!-- Return to shop -->
                                        <div class="col-6">
                                        ...
                                        <div class="col-6 text-right">
                                            <button type="button" onclick="submitOrder(this)" id="submitOrderBtn" class="btn btn-primary fw-600">{{ translate('Complete Order') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            @if (Auth::check())
                ...
                $.ajax({
                    ...
                    processData: false,
                        ...
                        $("#cart_summary_inner").html(data.html);
                        }
                    });
            @else
...
showUserTypeModal(); // Show user type selection modal
        ...
        function updateDeliveryAddress(id, city_id = 0, area_id=0) {
            ...
            $.post('{{ route('checkout.updateDeliveryAddress') }}', {
                ...
                $('#cart_summary_inner').html(data.cart_summary);
        ...
        // modern shipping info handles delivery/pickup toggling within its own script
