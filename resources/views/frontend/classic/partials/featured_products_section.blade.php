@php
    $featuredProducts = get_featured_products()->take(12);
@endphp
    ...
    <section class="products-section section_featured">
        <div class="container-full">
            <div class="section-header">
                ...
                {{-- View More link updated here (already correct) --}}
                <a href="{{ route('search', ['featured' => 1]) }}" class="view-more-btn">{{ translate('View More') }}</a>
            </div>

            <div class="products-grid">
                @foreach ($featuredProducts as $product)
                    {{-- এখানে $featuredProducts ব্যবহার করা হয়েছে --}}
                    @php
                    ...
                    <a href="{{ $url }}" class="product-card" title="{{ $name }}">
                        <div class="product-image">
                            <img src="{{ static_asset('assets/img/placeholder.jpg') }}" data-src="{{ $thumb }}"
                                class="lazyload" alt="{{ $name }}">
