...
<a href="javascript:void(0)" class="d-flex align-items-center text-dark h-100" data-toggle="dropdown" data-display="static"
    title="{{ translate('Cart') }}" style="margin-left: 70px;">
    <span class="mr-2">
        ...
        <ul class="h-360px overflow-auto c-scrollbar-light list-group list-group-flush mx-1">
            @foreach ($carts as $key => $cartItem)
                @php
                    ...
                    $product_name_with_choice = $product ? $product->getTranslation('name') : 'Product Not Found';
                    if ($product && $cartItem['variant_name'] != null) {
                        $product_name_with_choice =
                            $product->getTranslation('name') . ' - ' . $cartItem['variant_name'];
                    } elseif ($product && $cartItem['variation'] != null) {
                        $product_name_with_choice = $product->getTranslation('name') . ' - ' . $cartItem['variation'];
                    }

                    // Mohammad Hassan - Check if product is out of stock and can be preordered
                    $isOutOfStock = $product ? $product->isOutOfStock() : false;
                    $isPreorderAvailable = $product ? $product->isPreorderAvailable() : false;
                @endphp
                @if ($product != null)
                            ...
                            <a href="{{ route('product', $product->slug) }}"
                                ...
                                <span class="minw-0 pl-2 flex-grow-1">
                                    <span class="fw-700 fs-13 text-dark mb-2 text-truncate-2"
                                        title="{{ $product_name_with_choice }}">
                                        {{ $product_name_with_choice }}
                                    </span>
                                    @if ($isOutOfStock && $isPreorderAvailable)
                                        <div class="mb-1">
                                            <span class="text-warning fs-10 px-1 border border-warning rounded"
                                            ...
                                            <div class="text-info fs-10 mt-1">
                                                ...
                                                {{ translate('Est. Availability') }}:
                                                {{ \Carbon\Carbon::now()->addDays(14)->format('M d') }}
                                            </div>
                                            <div class="text-muted fs-10">
                                                <i class="las la-info-circle"></i>
                                                {{ translate('Advance payment required') }}
                                            </div>
                                        </div>
                                    @endif
