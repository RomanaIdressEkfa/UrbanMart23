...
@section('panel_content')
    ...
    @endphp
    <div class=" shadow-sm p-4 mb-4 text-center" style="background: #6A72DA; color: #fff; border: none;">
        <div class="d-flex flex-column align-items-center">
            <div class="mb-2" style="background: rgba(255,255,255,.2); border-radius: 9999px; padding: 6px 12px; font-weight: 700; font-size: 12px; letter-spacing: .5px;">
                {{ $isWholesaler ? translate('Wholesaler') : translate('Customer') }}
            </div>
            <h2 class="fw-700" style="font-size: 20px; margin-bottom: 8px;">
                {{ $isWholesaler ? translate('Welcome to Wholesaler Dashboard') : translate('Welcome to Customer Dashboard') }}
            </h2>
            <p class="mb-0" style="opacity: .95;">
                {{ translate('Signed in as') }}
                <strong>{{ $name ? $name : $email }}</strong>
                @if($name && $email)
