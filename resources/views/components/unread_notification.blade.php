...
@forelse($notifications as $notification)
    <li class="list-group-item d-flex justify-content-between align-items- py-3" @if($notification->type == 'App\Notifications\PreorderNotification') style="background-color: #fff3cd; border-left: 3px solid #ffc107;" @endif>
                ...
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        @elseif ($notification->type == 'App\Notifications\PreorderNotification' && addon_is_activated('preorder'))
                            @php
                                $orderCode  = $notification->data['order_code'];
                                $orderCode = "<span class='text-blue font-weight-bold'>".$orderCode."</span>";
                                $notifyContent = str_replace('[[order_code]]', $orderCode, $notifyContent);
                            @endphp
                        @endif
                            ...
                            <p class="mb-1 text-dark text-truncate-2">
                                @if ($notification->type == 'App\Notifications\PreorderNotification')
                                    <span class="badge badge-warning badge-sm mr-2">
                                        <i class="las la-clock"></i> Pre-Order
                                    </span>
                                @endif
                                {!! $notifyContent !!}
                            </p>
