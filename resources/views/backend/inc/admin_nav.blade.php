                            ...
                            <span class="btn btn-topbar btn-circle btn-light p-0 d-flex justify-content-center align-items-center" data-toggle="tooltip" data-title="{{ translate('Notification') }}">
                                <span class="d-flex align-items-center position-relative">
                                    ...
                                    @php
                                        ...
                                        $preorderUnread = auth()->user()->unreadNotifications()->where('type', 'App\Notifications\PreorderNotification')->count();
                                    @endphp
                                    @if ($totalUnread > 0)
                                        <span class="badge badge-sm badge-dot badge-circle badge-danger position-absolute absolute-top-right"></span>
                                        @if ($preorderUnread > 0)
                                            <span class="badge badge-sm badge-circle badge-warning position-absolute" style="top: -8px; right: 8px; font-size: 9px; min-width: 16px; height: 16px; line-height: 14px;">{{ $preorderUnread }}</span>
                                        @endif
                                    @endif
                                </span>
                            </span>
                                    ...
                                    @if (addon_is_activated('preorder'))
                                        <li class="nav-item">
                                            <a class="nav-link text-dark" data-toggle="tab" data-type="preorder" href="javascript:void(0);"
                                                data-target="#preorders-notifications" role="tab" id="preorders-tab">
                                                {{ translate('Preorders') }}
                                                @if ($preorderUnread > 0)
                                                    <span class="badge badge-warning badge-sm ml-1">{{ $preorderUnread }}</span>
                                                @endif
                                            </a>
                                        </li>
                                    @endif
