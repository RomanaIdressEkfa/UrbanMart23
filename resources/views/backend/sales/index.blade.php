                ...
                @canany(['delete_order', 'export_order'])
                    <div class="dropdown mb-2 mb-md-0">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            @endcan
                            @if(auth()->user()->can('unpaid_order_payment_notification') && isset($unpaid_order_payment_notification) && $unpaid_order_payment_notification->status == 1 && Route::currentRouteName() == 'unpaid_orders.index')
                    ...
                    <tbody>
                        @foreach ($orders as $key => $order)
                            <tr>
                                ...
                                <td>
    @if ($order->is_preorder_item)
        {{-- প্রি-অর্ডারের ক্ষেত্রে, আমরা ধরে নিতে পারি প্রতিটি অর্ডারে একাধিক আইটেম থাকতে পারে,
             তবে আপাতত সহজ রাখার জন্য '1' দেখানো হচ্ছে --}}
        1
    @else
        {{-- রেগুলার অর্ডারের জন্য orderDetails গণনা করা হচ্ছে --}}
...
<td>
    @if ($order->user != null)
        ...
        <br>
        <span class="text-secondary">{{ $order->user->email }}</span>
        <br>
        <span class="text-secondary">{{ $order->user->phone }}</span>
        <br>
        @if ($order->user->user_type == 'customer')
            <span class="badge badge-inline badge-success">{{ translate('Customer') }}</span>
        @elseif ($order->user->user_type == 'seller')
            <span class="badge badge-inline badge-info">{{ translate('Wholesaler') }}</span>
        @else
             <span class="badge badge-inline badge-primary">{{ ucfirst($order->user->user_type) }}</span>
        @endif
    @else
        {{-- --- সমাধান: যদি অতিথি ব্যবহারকারী হন --- --}}
        @php
            // shipping_address (JSON) থেকে তথ্য ডিকোড করা হচ্ছে
        ...
        @if ($shipping_address != null)
            ...
            <br>
            <span class="text-secondary">{{ $shipping_address->email }}</span>
            <br>
            <span class="text-secondary">{{ $shipping_address->phone }}</span>
            <br>
            <span class="badge badge-inline badge-secondary">{{ translate('Guest') }}</span>
        @else
            {{-- যদি কোনো কারণে shipping_address না পাওয়া যায় --}}
            <span>Guest (ID: {{ $order->guest_id ?? 'N/A' }})</span>
        @endif
    @endif
</td>
                                <td>
    ...
    <div id="complete_unpaid_order_payment" class="modal fade">
        <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 540px;">
            <div class="modal-content pb-2rem px-2rem">
                ...
                <form class="form-horizontal" action="{{ route('bulk-unpaid-order-payment-notification') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
@section('script')
    <script type="text/javascript">
        ...
        function order_bulk_export (){
            var url = '{{route('orders.bulk_export')}}';
