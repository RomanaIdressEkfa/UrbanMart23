    ...
    AIZ.plugins = {
        ...
        textEditor: function () {
            $(".aiz-text-editor").each(function (el) {
                ...
                // Guard to prevent double insertion when paste triggers both onPaste and onImageUpload
                var suppressImageUploadOnce = false;
                ...
                $this.summernote({
                    ...
                    callbacks: {
                        onImageUpload: function (files) {
                            // Skip once if paste already handled
                            if (suppressImageUploadOnce) {
                                suppressImageUploadOnce = false;
                                return;
                            }
                            if (!files || files.length === 0) return;
                            var file = files[0];
                            var formData = new FormData();
                            formData.append('aiz_file', file);
                            $.ajax({
                                ...
                                success: function(res){
                                    try {
                                        var url = (res && res.url) ? res.url : ((res && res.file_name) ? (AIZ.data.appUrl + '/' + res.file_name) : null);
                                        if (url) {
                                            $this.summernote('insertImage', url);
                                        } else {
                                            AIZ.plugins.notify('danger', 'Image upload failed');
                                        }
                                    } catch (e) {
                                        AIZ.plugins.notify('danger', 'Image upload error');
                                    }
                                },
                                error: function(){
                                    AIZ.plugins.notify('danger', 'Image upload failed');
                        ...
                        onPaste: function (e) {
                            if (format) {
                                ...
                                if (clipboardData && clipboardData.items) {
                                    // Only process first image file to avoid duplicates
                                    for (var i = 0; i < clipboardData.items.length; i++) {
                                        ...
                                        if (item.kind === 'file') {
                                            ...
                                            if (file) {
                                                e.preventDefault();
                                                // Signal to skip the subsequent onImageUpload callback
                                                suppressImageUploadOnce = true;
                                                var formData = new FormData();
                                                formData.append('aiz_file', file);
                                                $.ajax({
                                                    url: AIZ.data.appUrl + "/aiz-uploader/upload",
                                                    ...
                                                    success: function(res){
                                                        ...
                                                        if (url) {
                                                            ...
                                                            AIZ.plugins.notify('danger', 'Image paste failed');
                                                        }
                                                    },
                                                    error: function(){
                                                        AIZ.plugins.notify('danger', 'Image paste failed');
